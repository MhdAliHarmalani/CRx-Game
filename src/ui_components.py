...
class GameUI:
    ...
    def draw_player_indicator(self, screen: pygame.Surface,
    ...
    def draw_controls_info(self, screen: pygame.Surface) -> None:
        ...
        controls = ["Click cells to add orbs", "Reach critical mass to explode"]
        
        for i, control in enumerate(controls):
            text_surface = self.small_font.render(
                control, True, (180, 180, 180)
            )
            x_pos = (
                self.window_size[0] - text_surface.get_width() - UI_PADDING
            )
            y_pos = (
                self.window_size[1] -
